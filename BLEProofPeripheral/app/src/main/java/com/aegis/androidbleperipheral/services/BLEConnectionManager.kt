...
//            writeBLECharacteristic(mDataMDLPForMissedConnection);
    ...
    fun writeEmergencyGatt(value: String) {
        val mDataMDLPForEmergency =  BluetoothGattCharacteristic(java.util.UUID.
    ...
    fun findBLEGattService(mContext: Context) {
        ...
        if (serviceList != null) {
            for (gattService in serviceList) {
                if (gattService.uuid.toString().equals(mContext.getString(R.string.char_uuid_emergency),
                                ignoreCase = true)) {
